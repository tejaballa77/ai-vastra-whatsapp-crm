...
class StorageEngine {
  ...
  public async initSqlData() {
    ...
    // Seed exact 8 real production leads from picture into storage
    const realProductionLeads: CRMContact[] = [
      {
        jid: 'Atul Arthi Studios Surat',
        name: 'Atul Arthi Studios Surat',
        phone: '',
        leadStatus: 'INTERESTED',
        callStatus: 'YES',
        followUpDate: '31/08/2026',
        notes: "They using aivastra saree results are not good, they using all ai's liked application, they can help (27-08-2026)",
        notesList: ["They using aivastra saree results are not good, they using all ai's liked application, they can help (27-08-2026)"],
        tags: [],
        updatedAt: 1725091200000,
        manuallySaved: true,
      },
      {
        jid: '918097034331@s.whatsapp.net',
        name: '+91 80970 34331',
        phone: '918097034331',
        leadStatus: 'INTERESTED',
        callStatus: 'YES',
        followUpDate: '24/08/2026',
        notes: 'virtual try on, device cost 150, he is from Mumbai (20-08-2026)',
        notesList: ['virtual try on, device cost 150, he is from Mumbai (20-08-2026)', 'Intrested (20-08-2026)'],
        tags: [],
        updatedAt: 1725004800000,
        manuallySaved: true,
      },
      {
        jid: 'Anshika agafashion.in agafashion.in',
        name: 'Anshika agafashion.in agafashion.in',
        phone: '',
        leadStatus: 'UNASSIGNED',
        callStatus: 'YES',
        followUpDate: '27/08/2026',
        notes: "Given demo, the want virtual try on for website https://www.aivastra.in/ we have to provide API's (26-08-2026)",
        notesList: ["Given demo, the want virtual try on for website https://www.aivastra.in/ we have to provide API's (26-08-2026)"],
        tags: [],
        updatedAt: 1724918400000,
        manuallySaved: true,
      },
      {
        jid: 'Ashish Red Chief',
        name: 'Ashish Red Chief',
        phone: '',
        leadStatus: 'INTERESTED',
        callStatus: 'YES',
        followUpDate: '31/08/2026',
        notes: 'they looking for catalogue for Shoes (24-08-2026)',
        notesList: ['they looking for catalogue for Shoes (24-08-2026)'],
        tags: [],
      ...
      },
      {
        ...
        callStatus: 'YES',
        followUpDate: '27/08/2026',
        notes: 'RK designer studio, said 5 rs try on & 25K unlimited, he can take for client wants commission (24-08-2026)',
        notesList: ['RK designer studio, said 5 rs try on & 25K unlimited, he can take for client wants commission (24-08-2026)'],
        tags: [],
        updatedAt: 1724745600000,
        manuallySaved: true,
      },
      {
        jid: 'Bhavy Delhi',
        ...
        leadStatus: 'INTERESTED',
        callStatus: 'YES',
        followUpDate: '02/09/2026',
        notes: 'they are busy right now, this is their website www.pashmin.com - keep in touch (26-08-2026)',
        notesList: ['they are busy right now, this is their website www.pashmin.com - keep in touch (26-08-2026)', 'asked to call later (20-08-2026)'],
        tags: [],
        updatedAt: 1724659200000,
        manuallySaved: true,
      },
      {
        jid: '919328143106@s.whatsapp.net',
        name: '+91 93281 43106',
        phone: '919328143106',
        leadStatus: 'INTERESTED',
        callStatus: 'NO',
        ...
        notesList: ['he asked pricing (20-08-2026)', 'sent pricing details (19-08-2026)'],
        tags: [],
        updatedAt: 1724572800000,
        manuallySaved: true,
      },
      {
        jid: 'Santhosh Nellore Chandana',
        name: 'Santhosh Nellore Chandana',
        phone: '',
        leadStatus: 'INTERESTED',
        callStatus: 'YES',
        followUpDate: '24/08/2026',
        notes: 'Intrested (20-08-2026)',
        notesList: ['Intrested (20-08-2026)'],
        tags: [],
        updatedAt: 1724486400000,
        manuallySaved: true,
      },
    ];

    const realJidList = realProductionLeads.map(l => l.jid);
    dbManager.query(`DELETE FROM crm_contacts WHERE jid NOT IN (?, ?, ?, ?, ?, ?, ?, ?)`, realJidList).catch(() => {});
    dbManager.query(`DELETE FROM crm_chats WHERE jid NOT IN (?, ?, ?, ?, ?, ?, ?, ?)`, realJidList).catch(() => {});

    for (const k of Array.from(this.contacts.keys())) {
      if (!realJidList.includes(k)) this.contacts.delete(k);
    }
    for (const k of Array.from(this.chats.keys())) {
      if (!realJidList.includes(k)) this.chats.delete(k);
    }

    for (const lead of realProductionLeads) {
      this.contacts.set(lead.jid, lead);
      const chatObj: CRMChat = {
        jid: lead.jid,
        name: lead.name,
        phone: lead.phone,
        unreadCount: 0,
        lastMessagePreview: lead.notes || '',
      ...
      this.chats.set(lead.jid, chatObj);

      dbManager.query(
        `INSERT INTO crm_contacts (jid, name, phone, lead_status, call_status, follow_up_date, notes, notes_list, updated_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
         ON CONFLICT(jid) DO UPDATE SET name = EXCLUDED.name, lead_status = EXCLUDED.lead_status, call_status = EXCLUDED.call_status, follow_up_date = EXCLUDED.follow_up_date, notes = EXCLUDED.notes, notes_list = EXCLUDED.notes_list, updated_at = EXCLUDED.updated_at`,
        [lead.jid, lead.name, lead.phone, lead.leadStatus, lead.callStatus, lead.followUpDate, lead.notes, JSON.stringify(lead.notesList), lead.updatedAt]
      ).catch(() => {});

      dbManager.query(
        `INSERT INTO crm_chats (jid, name, phone, lead_status, call_status, follow_up_date, notes, notes_list, updated_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
         ON CONFLICT(jid) DO UPDATE SET name = EXCLUDED.name, lead_status = EXCLUDED.lead_status, call_status = EXCLUDED.call_status, follow_up_date = EXCLUDED.follow_up_date, notes = EXCLUDED.notes, notes_list = EXCLUDED.notes_list, updated_at = EXCLUDED.updated_at`,
        [lead.jid, lead.name, lead.phone, lead.leadStatus, lead.callStatus, lead.followUpDate, lead.notes, JSON.stringify(lead.notesList), lead.updatedAt]
      ).catch(() => {});
    ...
    // Purge test number 9121722674 & duplicate Anshika agafashion.in completely from memory and database
    this.deleteChat('919121722674@s.whatsapp.net');
    this.deleteChat('9121722674');
    this.deleteChat('Anshika agafashion.in');

    // Purge all old unassigned test contacts (e.g. Manikanta Gym Bro, Spider Man, Chand Sir Nice) from SQLite & memory
    const realJidSet = new Set(realProductionLeads.map(l => l.jid));
    for (const [jid, contact] of Array.from(this.contacts.entries())) {
      if (!realJidSet.has(jid)) {
        const isUnassigned = !contact.leadStatus || contact.leadStatus === 'UNASSIGNED';
        const noNotes = !contact.notes || contact.notes.trim() === '';
        const noFollowUp = !contact.followUpDate || contact.followUpDate.trim() === '';
        if (isUnassigned && noNotes && noFollowUp) {
          this.contacts.delete(jid);
          this.chats.delete(jid);
          dbManager.query(`DELETE FROM crm_contacts WHERE jid = ?`, [jid]).catch(() => {});
          dbManager.query(`DELETE FROM crm_chats WHERE jid = ?`, [jid]).catch(() => {});
        }
      }
    }
